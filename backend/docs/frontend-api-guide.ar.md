...
| `ENROLLMENT_UNAVAILABLE` | تعذر التسجيل؛ لا تفترض هل السبب هو الهوية أو وجود حساب/بريد سابق. |

| Status | التعامل في الواجهة |
|---|---|
| `400` | استخدم `code` و`fieldErrors` للعرض، مع إبقاء `ENROLLMENT_UNAVAILABLE` رسالة عامة. |
